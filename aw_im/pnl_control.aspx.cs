    ...
    public partial class pnl_control : System.Web.UI.Page
    {
        ...
        protected void btn_i_cnt_cp_Click(object sender, EventArgs e)
        {
            ...
            using (bd_imEntities db_sepomex = new bd_imEntities())
            {
                ...
                i_cnt_scolonia.DataSource = tbl_sepomex;
                i_cnt_scolonia.DataTextField = "d_asenta";
                i_cnt_scolonia.DataValueField = "id_asenta_cpcons";
                i_cnt_scolonia.DataBind();

                if (tbl_sepomex.Count == 1)
                {
                    i_cnt_municipio.Value = tbl_sepomex[0].d_mnpio;
                    i_cnt_estado.Value = tbl_sepomex[0].d_estado;
                    i_cnt_scolonia.Attributes.Add("required", "required");
                ...
                else if (tbl_sepomex.Count == 0)
                {
                    ...
                    i_cnt_scolonia.Items.Insert(0, new ListItem("Colonia", "0"));
                    i_cnt_municipio.Value = null;
                    i_cnt_estado.Value = null;
                    i_cnt_scolonia.Attributes.Add("required", string.Empty);
                ...
                try
                {
                    ...
                    using (bd_imEntities m_cnt = new bd_imEntities())
                    {
                        var i_cnt = (from i_c in m_cnt.inf_centro
                                     ...
                                     join i_l in m_cnt.fact_licencia on i_c.licencia_ID equals i_l.licencia_ID
                                     join i_up in m_cnt.inf_usr_personales on i_cc.usuario_ID equals i_up.usuario_ID

                                     where i_c.centro_ID == cnt_f
                                     select new
                                     {
                                         i_c.centro_ID,
                                         i_c.licencia_ID,
                                         ...
                                         i_up.apaterno,
                                         i_up.amaterno,
                                         nom_gerente = i_up.nombres + " " + i_up.apaterno + " " + i_up.amaterno,
                                         i_c.registro
                                     }).OrderBy(x => x.cod_centro).ToList();

                        if (i_cnt.Count == 0)
                        {
                            gv_cnt.DataSource = i_cnt;
                            gv_cnt.DataBind();
                            gv_cnt.Visible = true;
                            gv_cnt.Visible = true;
                        ...
                        i_cnt_nombres.Value = i_cnt[0].nombres;
                        i_cnt_apaterno.Value = i_cnt[0].apaterno;
                        i_cnt_amaterno.Value = i_cnt[0].amaterno;
                        i_cnt_nom.Value = i_cnt[0].centro_nom;
                        i_cnt_email.Value = i_cnt[0].email;
                        i_cnt_tel.Value = i_cnt[0].telefono;
                        i_cnt_callenum.Value = i_cnt[0].callenum;
                        i_cnt_cp.Value = i_cnt[0].d_codigo;

                        i_cnt_slicencia.Items.Clear();
                        i_cnt_scolonia.Items.Clear();

                        var i_ma = (from c in m_cnt.fact_licencia
                        ...
                        i_cnt_slicencia.Value = i_cnt[0].licencia_ID.ToString();

                        string str_dcodigo = i_cnt[0].d_codigo.ToString();

                        var tbl_sepomex = (from c in m_cnt.inf_sepomex
                        ...
                        i_cnt_scolonia.Items.Insert(0, new ListItem("Colonia", string.Empty));
                        i_cnt_scolonia.Value = i_cnt[0].id_asenta_cpcons.ToString();
        ...
        protected void gv_cnt_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                using (bd_imEntities m_est = new bd_imEntities())
                {
                    ...
                    est_ID = int.Parse(i_est.est_cnt_ID.ToString());

                    DropDownList ddl_est = (e.Row.FindControl("ddl_cnt_estatus") as DropDownList);

                    var tbl_sepomex = (from c in m_est.fact_est_cnt
                                       select c).ToList();

                    ddl_est.DataSource = tbl_sepomex;

                    ddl_est.DataTextField = "est_cnt_desc";
                    ddl_est.DataValueField = "est_cnt_ID";
                    ddl_est.DataBind();
                    ddl_est.Items.Insert(0, new ListItem("Seleccionar", string.Empty));
                    ddl_est.SelectedValue = est_ID.ToString();
                }
            }
        }
        ...
        protected void lkb_cnt_buscar_Click(object sender, EventArgs e)
        {
            ...
            if (string.IsNullOrEmpty(i_cnt_buscar.Text))
            {
            }
            else
            {
                ...
                if (f_busqueda == "TODO")
                {
                    ...
                    using (bd_imEntities m_cnt = new bd_imEntities())
                    {
                        var i_cnt = (from i_c in m_cnt.inf_centro
                                     ...
                                     join i_cc in m_cnt.inf_centro_ctrl on i_c.centro_ID equals i_cc.centro_ID
                                     join i_up in m_cnt.inf_usr_personales on i_cc.usuario_ID equals i_up.usuario_ID
                                     join i_uh in m_cnt.inf_usr_rh on i_cc.usuario_ID equals i_uh.usuario_ID
                                     where f_usr_c.Contains(i_uh.perfil_ID)
                                     where i_c.centro_tipo_ID == 2
                                     select new
                                     {
                                         i_c.centro_ID,
                                         i_l.licencia_desc,
        ...
        private void edita_centro(Guid cnt_f, string dd_cnt_nom, Guid i_licencia, string i_email, string i_tel, string dd_callenum, string i_cp, string i_colonia, string dd_nombres, string dd_apaterno, string dd_amaterno)
        {
            ...
            //foreach (GridViewRow row in gv_cnt.Rows)
            //{
            //    if (row.RowType == DataControlRowType.DataRow)
            //    {
            //        DropDownList dl = (DropDownList)row.FindControl("ddl_cnt_estatus");
            ...
            //    i_inv.est_cnt_ID = int_ddl;
            //    i_inv.centro_nom = dd_cnt_nom;
            //    i_inv.licencia_ID = i_licencia;
            //    i_inv.email = i_email;
            //    i_inv.telefono = i_tel;
            //    i_inv.callenum = dd_callenum;
            //    i_inv.d_codigo = i_cp;
            ...
            //    var i_prov_cont = (from c in m_inv.inf_usuario
            //                       join i_ei in m_prov.fact_est_prov on i_i.est_prov_ID equals i_ei.est_prov_ID
            //                       where c.prov_ID == prov_f
            //                       select c).FirstOrDefault();
            ...
            //ctrl_prov();
            //div_i_prov.Visible = false;
            Mensaje("Desarrollo pendiente!!! (reglas de negocio para la actualización de datos del centro y su gerente)");
        }

        private void guarda_centro(string dd_cnt_nom, Guid i_licencia, string i_email, string i_tel, string dd_callenum, string i_cp, string i_colonia, string dd_nombres, string dd_apaterno, string dd_amaterno, string cod_usr, string dd_clave, int i_ap)
        {
            Guid guid_emp;
            ...
            using (bd_imEntities m_usr = new bd_imEntities())
            {
                ...
                {
                    cod_usrf = "USR" + string.Format("{0:0000}", i_f_b.Count + 1);
                }

                var i_cnt = (from c in m_usr.inf_centro
                             select c).ToList();
                ...
                {
                    cod_cntf = "CNT" + string.Format("{0:0000}", i_cnt.Count + 1);
                }
                ...
                var i_f_bf = (from c in m_usr.inf_usuario
                              where c.usr == cod_usr
                              select c).ToList();
